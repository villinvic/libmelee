...
class Console:
    ...
    def run(self,
        ...
        command = [get_exe_path(self.path)]
    ...
    def __handle_slippstream_menu_event(self, event_bytes, gamestate: GameState):
        ...
        if scene == 0x02:
            ...
            # for i in range(0, 90):
            #     try:
            #         print(i, np.ndarray((1,), ">B", event_bytes, i)[0])
        ...
        if gamestate.menu_state in [enums.Menu.CHARACTER_SELECT, enums.Menu.SLIPPI_ONLINE_CSS]:
            ...
            try:
                gamestate.players[1].costume = np.ndarray((1,), ">B", event_bytes, 63)[0]
            except TypeError:
                gamestate.players[1].costume = 0
            try:
                gamestate.players[2].costume = np.ndarray((1,), ">B", event_bytes, 64)[0]
            except TypeError:
                gamestate.players[2].costume = 0
            try:
                gamestate.players[3].costume = np.ndarray((1,), ">B", event_bytes, 65)[0]
            except TypeError:
                gamestate.players[3].costume = 0
            try:
                gamestate.players[4].costume = np.ndarray((1,), ">B", event_bytes, 66)[0]
            except TypeError:
                gamestate.players[4].costume = 0
